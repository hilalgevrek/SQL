...
SELECT B.store_id, A.product_id, 0 QUATITY  -- Quantity'e 0 atadýk
FROM product.product A
CROSS JOIN sale.store B
WHERE A.product_id NOT IN (  -- Yalnýzca stok tablosunda olmayýp product tablosunda olanlarla ilgileniyoruz
	SELECT product_id
	FROM product.stock
	)
...
FROM product.product
ORDER BY category_id, list_price  -- Deðerlerine göre sýralamasýný istedik
...
SELECT B.brand_name, B.brand_id, A.list_price
FROM product.product A, product.brand B
WHERE A.brand_id = B.brand_id  -- Eðer where içinde aggregate function kullanmamýz gerekseydi where kullanamazdýk onun yerine having kullanýrdýk.
ORDER BY B.brand_name
...
WHERE A.brand_id = B.brand_id
GROUP BY B.brand_name, B.brand_id
HAVING AVG(list_price) > 1000  -- Burada aggregate function kullanmamýz gerekti where kullanamayýz bu nedenle having kullandýk.
