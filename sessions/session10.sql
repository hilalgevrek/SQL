...
select	category_id, list_price,
		ROW_NUMBER() OVER(partition by category_id order by list_price ASC) Row_num,
		RANK() OVER(partition by category_id order by list_price ASC) Rank_num,
		DENSE_RANK() OVER(partition by category_id order by list_price ASC) Dense_Rank_num  -- DENSE_RANK'de 16. output 15 ile devam etmiş, rank'de 16 ile devam etmiş. 
from	product.product
order by category_id, list_price
;
...
SELECT	brand_id, list_price,  -- 3 dediğimiz yer round ile ilgili
		ROUND (CUME_DIST() OVER (PARTITION BY brand_id ORDER BY list_price) , 3) AS CUM_DIST  -- 'brand_id'si 1 ve fiyatı 7.99 ve altında olan ürünler, brand_id'si 1 olan tüm ürünlerin %2.4'ünü oluşturuyor' şeklinde yorum çıkarabilirim.
FROM	product.product																		-- Brad_id'nin son fiyatına geldiğinde cüm_dist 1 olur. Çünkü kümülatif bir toplam bu yüzden altındaki tüm değerleri de kapsıyor.
;																							-- Cüm_dist sayesinde ürünlerimin ilk %25'i 80 dolar altında gibi yorumlar çıkarmak mümkün hale gelir (örnek)
...
SELECT	brand_id, list_price, 
		...
		ROUND (PERCENT_RANK() OVER (PARTITION BY brand_id ORDER BY list_price) , 3) AS PERCENT_RANK  -- percent_rank = (row num-1)/(total-1)
FROM	product.product
;
...
SELECT	brand_id, list_price, 
		...
		NTILE(4) OVER (PARTITION BY brand_id ORDER BY list_price) AS NTILE_NUM,  -- Her bir partition'umu 4 kümeye ayırdım
		COUNT(*) OVER(PARTITION BY brand_id) AS UrunSayisi  -- Her bir markanın içinde kaç tane ürün olduğunu verir
FROM	product.product
;
